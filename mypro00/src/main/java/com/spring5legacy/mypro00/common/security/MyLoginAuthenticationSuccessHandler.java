...
public class MyLoginAuthenticationSuccessHandler
   extends SavedRequestAwareAuthenticationSuccessHandler {
	...
	public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
        ...
        System.out.println("로그인 성공-> 전달된 Authentication 내용: " + authentication);

        //권한이름을 Set 객체에 저장
        Set<String> authNames = AuthorityUtils.authorityListToSet(authentication.getAuthorities());
        System.out.println("권한이름목록: " + authNames); //권한이름의 리스트를 콘솔에 표시

 
        //과제: 아래의 HttpSessionRequestCache, SavedRequest 조사할 것
        RequestCache requestCache = new HttpSessionRequestCache();
        SavedRequest savedRequest = requestCache.getRequest(request, response);

        if(savedRequest == null) {
            if (authNames.contains("ADMIN")) {
